...
class test_partner_lastname(common.TransactionCase):

    def setUp(self):
        super(test_partner_lastname, self).setUp()
        ...
        self.fields_user = {
            'name': 'firstname', 'login': 'v5Ue4Tql0Pm67KX05g25A'}
    ...
    def test_copy_partner(self):
        ...
        vals = self.partner_model.read(cr, uid, [res_id], [
            'name', 'firstname', 'lastname'], context=context)[0]

        self.assertEqual(
            vals['name'],
            _('%s (copy)') % 'firstname' + " lastname",
        ...
        self.assertEqual(vals['lastname'], 'lastname',
    ...
    def test_copy_user(self):
        ...
        self.assertEqual(
            vals['firstname'], _('%s (copy)') %
            'firstname', 'Copy of the user failed with wrong firstname')
        self.assertEqual(vals['lastname'], 'lastname',
    ...
    def test_update_user_firstname(self):
        ...
        self.partner_model.write(
            cr, uid, flds['partner_id'][0], {'lastname': 'lastname'},
            context=context)
        self.user_model.write(
            cr, uid, res_id, {'name': 'change lastname'}, context=context)
        vals = self.user_model.read(
            cr, uid, [res_id], ['name', 'firstname', 'lastname'],
            context=context)[0]

        self.assertEqual(vals['name'], 'change lastname',
                         'Update of the user firstname failed with wrong name')
        self.assertEqual(
            vals['firstname'], 'change',
    ...
    def test_update_user_lastname(self):
        ...
        self.partner_model.write(
            ...
            context=context)
        self.user_model.write(
            cr, uid, res_id, {'name': 'firstname other'}, context=context)
        vals = self.user_model.read(
            cr, uid, [res_id], ['name', 'firstname', 'lastname'],
            context=context)[0]

        self.assertEqual(
            vals['name'], 'firstname other',
            'Update of the user lastname failed with wrong name')
        self.assertEqual(
            vals['firstname'], 'firstname other',
